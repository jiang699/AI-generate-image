...
export async function POST(request: NextRequest) {
  try {
    ...
    // 4. 创建生成记录
    const settings = { imageUrl, strength, width, height, steps, guidance, seed };
    const recordResult = await createGenerationRecord(user.id, prompt, 'image', settings);
    ...
    console.error('Image to image async API error:', error);
    return NextResponse.json(
